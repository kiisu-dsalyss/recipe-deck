import type { BloomSample, Dot } from "./floatingDotsSim.types";

export type { BloomSample, Dot } from "./floatingDotsSim.types";

export const DOT_COUNT = 80;
export const LINE_MAX_PX = 260;
/** Base scale for angular drift; multiplied by activity and per-dot frequencies. */
export const DRIFT_SPEED = 0.00012;
/** Low throughput / idle — slower than historical default. */
export const SPEED_MUL_MIN = 0.32;
...
export const SPEED_SMOOTH = 0.14;
/**
...
export const LINE_BRIGHTER = 1.14;

/** Map tok/s to drift speed multiplier (higher load → faster motion). */
export function activitySpeedMultiplier(tok: number | null | undefined): number {
 ...
 */
export function canvasLightness(l: number, lightMode: boolean): number {
  if (!lightMode) {
    return l;
  }
...
export function drawPointerBloom(
 ...
 */
export function idleFocalTargetNorm(
  elapsed: number,
  seedA: number,
  seedB: number,
): { nx: number; ny: number } {
...
export function connectorLineStyle(d: Dot): {
  lineSat: number;
  lineLight: number;
